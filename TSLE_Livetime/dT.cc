...
int main (int argc, char* argv[])
{
    ...
    if(dataType == 0 /* BG data */){
        ...
        for(int subset_i = 0; subset_i <= dsSubsetMax; subset_i++) {LoadDataSet(ds, DS, subset_i);}
        cout << "The following GetGatifiedChain seems to be failing"<< endl;
    ...
    TFile* f = new TFile(Form("DS%d_dT_%d.root",DS,dataType),"RECREATE");
