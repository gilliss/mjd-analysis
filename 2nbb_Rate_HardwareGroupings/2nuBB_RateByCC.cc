...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // INITIALIZATIONS
    int maxMod = 2, maxPos = 7, maxDet = 5, maxCC = 4;
    char canvName[50];
    char graphName[50];
    ...
    double ccInt = 0;
    double norm_ccInt = 0;
    double enorm_ccInt = 0;
    /////double ccMass = 0;
    int ccList[4];
    int chMapDS = DS; if(DS==51 || DS==52){chMapDS=5;} // probably need the CC mappings for other DSs
    if(chMapDS==5 && c==1) {
        ccList[0] = 6; ccList[1] = 7; ccList[2] = 9; ccList[3] = 10;
        ccNameVec.push_back("6");
        ccNameVec.push_back("7");
        ccNameVec.push_back("9");
        ccNameVec.push_back("10");
    }
    if(chMapDS==5 && c==2) {
        ccList[0] = 5; ccList[1] = 9; ccList[2] = 13; ccList[3] = 17;
        ccNameVec.push_back("5");
        ccNameVec.push_back("9");
        ccNameVec.push_back("13");
        ccNameVec.push_back("17");
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(int cutScheme = 0; cutScheme <= 3; cutScheme++)
    {
        ...
        for (int CC = 0; CC <= (maxCC-1); CC++)
        {
            ...
            while(inTextFile >> m >> p >> d >> dInt >> dIntN >> dEX >> dEXU)
            {
                if(chMap->GetInt(m,p,d,"kDetectorType")==enrType)
                {
                    if(chMap->GetInt(m,p,d,"kPreAmpDigitizer") == ccList[CC])
                    {
                        ...
                        norm_ccInt+=norm_dInt;
                        enorm_ccInt+=sqrt(dInt)*scale;
                        ccInt+=dInt;
                        /////ccMass+=dMkg;
                        nDets++;
                    }
            ...
            if(nDets!=0){
                norm_ccInt=norm_ccInt/nDets;
                enorm_ccInt=enorm_ccInt/nDets;
            }
            else{ // nDets == 0
                norm_ccInt=0.;
                enorm_ccInt=0.;
            }
            graph[cutScheme]->SetPoint(i,double(i+1),norm_ccInt);
            graph[cutScheme]->SetPointError(i,0.0,enorm_ccInt);
            i++;
            
            if(cutScheme==3 && nDets!=0){
                ccName = to_string(ccList[CC]);
                groupingFill.push_back(norm_ccInt);
    ...
    mg->Add(graph[2]);
    mg->Add(graph[3]);
    if(enrType==0)mg->SetTitle(Form("Rate 1000-1400keV, DS%d M%d Natural Detectors by CC",DS,c));
    if(enrType==2)mg->SetTitle(Form("Rate 1000-1400keV, DS%d M%d Enriched Detectors by CC",DS,c));
    mg->Draw("AP");
    mg->GetYaxis()->SetTitle("Cnt/kg/dy");
    if(enrType==0)mg->GetYaxis()->SetRangeUser(0.002,1.6);
    if(enrType==2)mg->GetYaxis()->SetRangeUser(0.5,2.0);
    mg->GetXaxis()->SetTitle("CC (kPreAmpDigitizer)");
    ...
    if(save==1) canv->Print(Form("DS%d_M%d_DetType%d_CC_Fit.pdf",DS,c,enrType));
