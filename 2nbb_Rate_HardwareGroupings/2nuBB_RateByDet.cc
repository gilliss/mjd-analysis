...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(int cutScheme = 0; cutScheme <= 3; cutScheme++)
    {
        ...
        while(inTextFile >> m >> p >> d >> dInt >> dIntN >> dEX >> dEXU)
        {
            if(chMap->GetInt(m,p,d,"kDetectorType")==enrType)
            {
                ...
                if(cutScheme == 0) {graph[cutScheme]->SetPoint(i,double(i+1),norm_dInt);}
                else {graph[cutScheme]->SetPoint(i,detCoord.find(cpdName)->second,norm_dInt);}
                graph[cutScheme]->SetPointError(i,0.0,enorm_dInt);
                i++;
    ...
    mg->Add(graph[2]);
    mg->Add(graph[3]);
    if(enrType==0)mg->SetTitle(Form("Rate 1000-1400keV, DS%d M%d Natural Detectors",DS,c));
    if(enrType==2)mg->SetTitle(Form("Rate 1000-1400keV, DS%d M%d Enriched Detectors",DS,c));
    mg->Draw("AP");
    mg->GetYaxis()->SetTitle("Cnt/kg/dy");
    if(enrType==0)mg->GetYaxis()->SetRangeUser(0.0,1.7);
    if(enrType==2)mg->GetYaxis()->SetRangeUser(0.5,2.7);
    TAxis* a = mg->GetXaxis();
    int j = 1;
    int bin_j;
...
//    legend->Draw();
    ...
    // AVERAGE RATE
    double avgrate = 0;
    cout << "size avgnormrate vec "<<avgnormrate.size()<<endl;
    for(int k = 0; k < avgnormrate.size(); k++)
    {
        cout<<"  k "<<k<<" "<<avgnormrate[k]<<endl;
        avgrate+=avgnormrate[k];
    }
    cout <<"avg norm rate cut scheme 3 = " << avgrate/double(avgnormrate.size()) << endl;
    ...
    if(save==1) canv->Print(Form("DS%d_M%d_DetType%d_Det_Fit.pdf",DS,c,enrType));
