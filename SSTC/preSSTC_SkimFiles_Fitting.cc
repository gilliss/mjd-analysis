...
int main(int argc, char* argv[])
{
    ...
    TFile * originalFile = TFile::Open("/Users/tomgilliss/Desktop/UNC/ENAP/Analysis/SSTC/preSSTC_Plots_P3JDYGolden_SkimFiles_Cleaned1st.root"); // Local
    ...
    for (int i=0; i<(nGoodChannels+3); i++)
    {
        ...
        meanToE = fit_ToE->GetParameter(1);
        sigmaToE = fit_ToE->GetParameter(2);
        if (sigmaToE<0.0) sigmaToE=-sigmaToE;
        */
        
        hLowE[i]->Draw();
        peakpos = 10.3; // guess peak by max bin in window
        peakheight = 0.0;; // get the height of the max bin
        fit_LowE->SetRange(8.8,11.3);//(0.53*peakpos,1.4*peakpos);
        ...
        meanLowE = fit_LowE->GetParameter(1);
        sigmaLowE = fit_LowE->GetParameter(2);
        if (sigmaLowE<0.0) sigmaLowE=-sigmaLowE;
        
        //cout<<"if(i=="<<i<<") "<<"{meanToE="<<meanToE<<"; sigmaToE="<<sigmaToE<<"; meanLowE="<<meanLowE<<"; sigmaLowE="<<sigmaLowE<<";}"<<endl;
        cout<<"if(i=="<<i<<") "<<"{meanLowE="<<meanLowE<<"; sigmaLowE="<<sigmaLowE<<";}"<<endl;
    }
    ...
    peakpos = 10.3; // guess peak by max bin in window
    peakheight = 0.0;; // get the height of the max bin
    fit_LowE->SetRange(8.8,11.3);//(0.54*peakpos,1.4*peakpos);
    fit_LowE->SetLineColor(kRed);
    fit_LowE->SetLineWidth(1);
    fit_LowE->SetLineStyle(2);
    fit_LowE->SetParameter(0,0.0); // gaus amplitude
    ...
    cToE->cd();
    cToE->SetLogy();
    hToE[nGoodChannels+2]->Draw();
    peakpos = hToE[nGoodChannels+2]->GetBinCenter(hToE[nGoodChannels+2]->GetMaximumBin()); // guess peak by max bin in window
    peakheight = hToE[nGoodChannels+2]->GetBinContent(hToE[nGoodChannels+2]->FindBin(peakpos)); // get the height of the max bin
    fit_ToE->SetLineColor(kRed);
    fit_ToE->SetLineWidth(1);
    fit_ToE->SetLineStyle(2);
    fit_ToE->FixParameter(0,peakheight*0.92); // gaus amplitude
    ...
    fit_ToE->SetParameter(3,0); // flat BG
    fit_ToE->SetParameter(4,0); // linear BG
    hToE[nGoodChannels+2]->Fit("ToEFit","qr+"); // for fit options https://root.cern.ch/doc/master/classTH1.html#a63eb028df86bc86c8e20c989eb23fb2a
    cToE->Update();
    
    double meanLowE = fit_LowE->GetParameter(1);
    double sigmaLowE = fit_LowE->GetParameter(2);
    double meanToE = fit_ToE->GetParameter(1);
    double sigmaToE = fit_ToE->GetParameter(2);
    
    cout<<"LowE: mean = " << meanLowE << " sigma = " << sigmaLowE << endl;
    cout<<"ToE: mean = " << meanToE << " sigma = " << sigmaToE << endl;
     */
