...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    vector<double>* trirt100nsft10nsMax = 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //TFile *f = new TFile("/Users/tomgilliss/Desktop/UNC/ENAP/Analysis/SSTC/data/preSSTC_Plots.root","NEW"); // Local
    TFile *f = new TFile("/global/u2/g/gilliss/dev/Analysis/Cryo1/SSTC/preSSTC_Plots_5126-5146.root","NEW"); // PDSF
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //ifstream runList ("/Users/tomgilliss/Desktop/UNC/ENAP/Analysis/SSTC/data/ShortRunList.txt"); // Local
    ifstream runList ("/global/u2/g/gilliss/dev/Analysis/Cryo1/SSTC/RunList_GoldenP3JDY_5126-5146.txt"); // PDSF
    //ifstream runList ("/global/u2/g/gilliss/dev/Analysis/Cryo1/SSTC/ShortRunList.txt"); // PDSF test
    ...
    if (runList.is_open())
    {
        while ( getline (runList,line,'\n') )  // http://www.cplusplus.com/forum/general/13087/
        {
            ...
            ////cout << "----------RUN " << i << "-------------------" << endl;
            runNumber = atoi(line.c_str()); // convert to int
            //cout << line << endl;

            sprintf(infile,"mjd_run%d",runNumber);
            //sprintf(infilename,"/Users/tomgilliss/Desktop/UNC/ENAP/Analysis/SSTC/data/%s.root",infile); // Local
            sprintf(infilename,"/global/project/projectdirs/majorana/data/mjd/surfmjd/data/gatified/P3JDY/%s.root",infile); // PDSF
            ...
            t->SetBranchAddress("run",&run); // set addresses for branches in the MJD Tree
            ...
            t->SetBranchAddress("trapECal",&trapECal);
            t->SetBranchAddress("trapETailMin",&trapETailMin);
            t->SetBranchAddress("trirt100nsft10nsMax",&trirt100nsft10nsMax);
            ...
            //cout << "Run " << runNumber << " entries " << nentriest << endl;
                ...
                if(channel->at(0)!=0) // check for bogus 0-value
                {
                    for(int j=0; j<chanSize; j++) // loop through channels in the entry
                    {
                        ...
                        if(CheckChannel(ch)!=1000) //if(ch%2==0) // use just high gain channels
                        {
                            ...
                            {
                                T = trirt100nsft10nsMax->at(j);
                                E = trapECal->at(j) - EnergyShift(ch);
                                hToEvE[chIndex]->Fill(E,T/E);
                                hToEvE[nGoodChannels+2]->Fill(E,T/E);
                                if(CheckEnriched(ch)==0) hToEvE[nGoodChannels+0]->Fill(E,T/E);
                                if(CheckEnriched(ch)==1) hToEvE[nGoodChannels+1]->Fill(E,T/E);
                                
                                if (E<=50.0)
                                {
                                    hLowE[chIndex]->Fill(E);
                                    hLowE[nGoodChannels+2]->Fill(E);
                                    if(CheckEnriched(ch)==0) hLowE[nGoodChannels+0]->Fill(E);
                                    if(CheckEnriched(ch)==1) hLowE[nGoodChannels+1]->Fill(E);
                                }
                            }
                        }
